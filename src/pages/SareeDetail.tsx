...
const SareeDetailPage = () => {
  ...
  useEffect(() => {
    ...
    // First check localStorage for the saree
    const savedSarees = localStorage.getItem(SAREES_STORAGE_KEY);
    let allSarees = sareesData;
    
    if (savedSarees) {
      allSarees = JSON.parse(savedSarees);
    }
    
    const foundSaree = allSarees.find(s => s.id === id);
    setSaree(foundSaree || null);
    
    // Related sarees (same category, excluding current)
    if (foundSaree) {
      const related = allSarees
        .filter(s => s.category === foundSaree.category && s.id !== foundSaree.id)
        .slice(0, 4);
  ...
  const handleWhatsAppClick = () => {
    ...
    const message = `Hi, I'm interested in purchasing the ${saree.name} (Price: ₹${saree.price.toLocaleString()}) from your website. Could you provide more details?`;
    const encodedMessage = encodeURIComponent(message);
    const whatsappUrl = `https://wa.me/919688484344?text=${encodedMessage}`;
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {/* Image Skeleton */}
          <div className="relative overflow-hidden rounded-lg bg-gray-200 animate-pulse h-96"></div>
          
          {/* Details Skeleton */}
          <div className="space-y-6">
            <div className="h-8 w-3/4 bg-gray-200 animate-pulse rounded mb-2"></div>
            <div className="h-6 w-1/4 bg-gray-200 animate-pulse rounded mb-8"></div>
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="relative overflow-hidden rounded-lg">
          <div 
            className={`cursor-zoom-in transition-transform duration-300 ${isImageZoomed ? 'scale-150' : 'scale-100'}`}
          ...
          >
            <img
              ...
              onError={(e) => {
                ...
                (e.target as HTMLImageElement).src = 'https://via.placeholder.com/600x800?text=Image+Not+Available';
              }}
            />
          </div>
          {isImageZoomed && (
        ...
        <div>
          ...
          <div className="prose prose-sm max-w-none mb-8">
            <p className="text-gray-700 mb-6">{saree.description}</p>
            <p className="text-gray-700 mb-6">{saree.details}</p>
          </div>
