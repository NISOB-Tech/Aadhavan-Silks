...
const HomePage = () => {
  ...
  useEffect(() => {
    const loadSarees = () => {
      ...
      if (savedSarees) {
        try {
          const parsedSarees = JSON.parse(savedSarees);
          setSarees(parsedSarees);
        } catch (error) {
          console.error("Error parsing sarees from localStorage:", error);
          setSarees(sareesData);
          // Re-initialize localStorage with mock data to fix corrupted data
          localStorage.setItem(SAREES_STORAGE_KEY, JSON.stringify(sareesData));
        }
      } else {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === SAREES_STORAGE_KEY && event.newValue) {
        try {
  ...
  return (
    <div>
      ...
      <section className="relative">
        <div className="h-[70vh] bg-gradient-to-r from-gold-700/90 to-burgundy-900/90 relative overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center px-4">
            <div className="text-center max-w-4xl">
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight">
                Exquisite Silk Sarees for Every Occasion
              </h1>
              <p className="text-xl text-white/90 mb-8">
                Discover the elegance and tradition of handcrafted silk sarees at Sri Aadhavan Silks
              </p>
              <Link
                to="/catalog"
                className="inline-block bg-white text-gold-700 font-medium px-8 py-3 rounded-md shadow-md hover:bg-gold-50 transition-colors"
              >
                Explore Collection
              </Link>
            </div>
      ...
      <section className="py-16 px-4 bg-silk-50">
        <div className="container mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold mb-6">About Sri Aadhavan Silks</h2>
              <p className="text-gray-700 mb-6">
                Sri Aadhavan Silks has been preserving and promoting the rich heritage of Indian silk
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 md:p-8">
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                ...
                <p className="text-gray-600 mb-6">
                  <strong>Email:</strong> info@sriaadhavanssilks.com
                </p>
